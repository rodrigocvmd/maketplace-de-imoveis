...
function CreateListing() {
	...
	const onSubmit = async (e) => {
		...
		if (discountedPrice >= regularPrice) {
			setLoading(false);
			toast.error("Preço da oferta deve ser menor que o preço normal");
			return;
		}
		...
		const imgUrls = await Promise.all([...images].map((image) => storeImage(image))).catch(() => {
			setLoading(false);
			toast.error("Imagens não enviadas");
			return;
		});
		...
		<div className="profile">
			...
			<main>
				<form onSubmit={onSubmit}>
					<label className="formLabel">Venda / Aluguel</label>
					...
					<label className="formLabel">Vaga para veículo</label>
					<div className="formButtons">
						<button
					...
					{/* TODO: */}
					{/* <h4 className="formLabel">Deseja inserir a localização no mapa?</h4>
					<p>
						Caso deseje adicionar a localização no mapa, insira abaixo as coordenadas do imóvel, que podem
						ser obtidas digitando o endereço
						<a style={{ color: "blue" }} target="_blank" href="https://www.google.com/maps">
							aqui.
						</a>
					</p> */}
					...
					<p className="imagesInfo">
						Clique em &quot;Choose files&quot; para adiconar as fotos do imóvel. <br />A primeira
						imagem será a capa do anúncio. Máximo de 6 fotos.
					</p>
					<input
